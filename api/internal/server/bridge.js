...
frame.addEventListener('load',()=>{const d=doc();if(!d)return;d.addEventListener('mouseup',()=>setTimeout(selection,0));d.addEventListener('keyup',()=>setTimeout(selection,0));d.addEventListener('click',activate);const targetOrigin=appOrigin==='self'?'*':appOrigin;parent.postMessage({type:'aidocs:ready'},targetOrigin)})
